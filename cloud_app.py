...
# -*- coding: utf-8 -*-
"""
cloud_app.py : le cloud KORKO de l'équipe, parti de cloud_exemple.py.

Ajout : chaque DEPART, RETOUR, ETRANGERE ou PERDUE validé ici est inscrit
dans le carnet de vie de la planche, sur Avalanche (voir korko_chain.py).
Sans web3 ou sans .env, la blockchain se met en veille et tout le reste
marche comme avant.

    python3 cloud_app.py        puis http://localhost:9000
...
chaine = None          # korko_chain.Chaine, créée au lancement
...
def inscrire(balise, type_, station, t):
    """Carnet de vie on-chain. Jamais bloquant, jamais de donnée client."""
    if chaine is not None:
        chaine.publier(balise, type_, station, t)
...
def retards():
    ...
    for balise, s in list(sessions.items()):
        ...
        if duree > PERDUE:
            ...
            inscrire(balise, "PERDUE", planches[balise]["origine"], horloge)
...
def traiter(ev):
    ...
    if type_ == "DEPART":
        ...
        p["sorties"] += 1
        inscrire(balise, "DEPART", station, ev["t"])
        ...
        inscrire(balise, type_, station, ev["t"])
...
PAGE_PASSEPORT = """<!doctype html><meta charset=utf-8>
<title>KORKO passeport</title>
<body style="font:14px ui-monospace,monospace;background:#EDE3CE;color:#1F6B6B;padding:1.5rem">
<pre>%s</pre></body>"""
...
def bloc_chaine():
    """Section Avalanche du tableau de bord, avec liens vers l'explorateur."""
    l = ["", "", "BLOCKCHAIN · Avalanche Fuji", "---------------------------"]
    if chaine is None or not chaine.actif:
        raison = chaine.raison if chaine is not None else "non démarrée"
    ...
    l.append('passeports : ' + "  ".join('<a href="/passeport?planche=%s">%s</a>' % (b, b)
                                         for b in sorted(planches)))
    for r in e["derniers"][:8]:
        l.append('%-9s %-9s %s  t=%d  <a href="%s">tx</a>'
                 % (r["balise"], r["type"], r["station"] or "-", r["t"], chaine.lien("tx", r["tx"])))
    return "\n".join(l)


def passeport(balise):
    """Le carnet de vie d'une planche, lu directement sur la chaîne."""
    if chaine is None or not chaine.actif:
        return "Blockchain en veille."
    try:
        p = chaine.lire_planche(balise)
    except Exception as e:
        return "Lecture impossible : %s" % html.escape(str(e))
    e = chaine.etat()
    l = ["PASSEPORT %s  (NFT #%d)" % (balise, p["id"]), "",
         "base            %s" % p["origine"],
         "statut          %s" % p["statut"],
         "sorties         %d" % p["sorties"],
         "événements      %d" % p["evenements"],
         "dernier         %s à t=%d" % (p["dernier"], p["dernier_t"]), "",
         '<a href="%s">voir le NFT sur l\'explorateur</a>'
         % chaine.lien("nft", "%s/%d" % (e["contrat"], p["id"])),
         '<a href="%s">tous les événements du contrat</a>' % chaine.lien("address", e["contrat"]),
         "", "Derniers envois de cette session :"]
    l += ['  %-9s %s t=%d <a href="%s">tx</a>' % (r["type"], r["station"] or "-", r["t"],
                                                 chaine.lien("tx", r["tx"]))
          for r in e["derniers"] if r["balise"] == balise] or ["  (aucun)"]
    l += ["", '<a href="/">retour au tableau de bord</a>']
    return "\n".join(l)
...
class Cloud(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path == "/chaine":                     # état de la blockchain, en JSON
            return self.repondre(json.dumps(chaine.etat() if chaine else {"actif": False},
                                            ensure_ascii=False, indent=1),
                                 "application/json; charset=utf-8")

        if u.path == "/passeport":                  # carnet de vie d'une planche
            b = q.get("planche", ["korko-01"])[0]
            if b not in planches:
                return self.repondre("Planche inconnue.", code=404)
            return self.repondre(PAGE_PASSEPORT % passeport(b), "text/html; charset=utf-8")
...
if __name__ == "__main__":
    from korko_chain import Chaine
    chaine = Chaine(note=note)
    ...
    print("  blockchain : /chaine     passeport : /passeport?planche=korko-01\n")
